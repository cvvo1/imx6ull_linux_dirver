...
struct gpioled_dev{
	...
	int dev_stats;  /* 设备状态：0，设备未使用。>0，设备以及被使用 */
	spinlock_t lock;  /* 自旋锁 */
...
static int led_open(struct inode *inode, struct file *filp)
{
	...
	spin_lock_irqsave(&gpioled.lock,flags);  /* 上锁，中间为临界保护区 */
	if(gpioled.dev_stats){   /* 驱动不可用 */
		spin_unlock_irqrestore(&gpioled.lock,flags); 
		return -EBUSY; /* 返回忙状态 */
	}
	gpioled.dev_stats++;  /* 标记被使用 */											
...
static int led_release(struct inode *inode, struct file *filp)
{
	...
	spin_lock_irqsave(&dev->lock,flags);  /* 上锁，中间为临界保护区 */
	if(&dev->dev_stats){   
		dev->dev_stats--;   /* 释放后，标记驱动未使用 */
	}											
